...
class ThreadsAPI:
    ...
    # 一時的な2枚画像投稿対応
# image_posting_fix.py の FixedImagePostingSystem クラスの create_multi_image_post メソッドを以下に修正
...
def create_multi_image_post(self, account, text, image_paths):
    ...
    try:
        ...
        # 一時的対応：1枚目の画像で投稿（2枚同時投稿は今後実装）
        print("🔄 一時的対応：1枚目の画像で投稿を作成")
        ...
        if post_result.success:
            print(f"✅ 2枚画像投稿成功（1枚目投稿 + 2枚目URL記載）: {post_result.post_id}")
            return PostResult(
                success=True,
...
def create_multi_image_post(self, account: Account, text: str, image_urls: List[str]) -> PostResult:
    """
    複数画像投稿作成（2枚画像対応）
    ...
    try:
        ...
        if response.status_code != 200:
            return PostResult(
                success=False,
                error=f"Multi-image post creation failed: HTTP {response.status_code}: {response.text}"
            )
        ...
        if not creation_id:
            return PostResult(
                success=False,
                error="No creation_id returned from multi-image API"
            )
        ...
        if publish_result.success:
            ...
            return PostResult(
                success=False,
                error=f"Multi-image publish failed: {publish_result.error}"
            )
            
    except Exception as e:
        return PostResult(
            success=False,
            error=f"Exception during multi-image post: {str(e)}"
        )
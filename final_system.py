...
class ThreadsAutomationSystem:
    ...
    def single_post(self, account_id=None, test_mode=False, custom_text=None):
        ...
        try:
            ...
            # CSV列名に対応するキーをチェック（「画像使用」または「use_image」）
            image_url = None
            is_image_post = (
                main_content.get('use_image') == 'YES' or 
                main_content.get('use_image') is True or
                main_content.get('画像使用') == 'YES' or
                main_content.get('image_use') == 'YES'
            )
    ...
    def upload_all_images(self):
        ...
        image_contents = []
        for content in self.content_system.main_contents:
            # 複数の可能なキー名をチェック
            if (content.get('use_image') == 'YES' or 
                content.get('use_image') is True or
                content.get('画像使用') == 'YES' or
                content.get('image_use') == 'YES'):
                image_contents.append(content)
        
        print(f"📊 画像付きコンテンツ: {len(image_contents)}件")
